"""
btc_research/eth_bot/connectors/mt5_connector.py — MT5 connector for ETH Bot.

Fully standalone — mirrors btc_bot_2/connectors/mt5_connector.py exactly,
but reads credentials and UTC offset from eth_bot.settings only.
...
This is critical for ETH Bot kill-zone logic:
  Kill-zone hours are UTC hours.
...
try:
    ...
    logger.warning("MetaTrader5 package not available — ETH MT5 connector in stub mode")
...
def connect(login: int = 0, password: str = "", server: str = "") -> bool:
    ...
    if not _MT5_AVAILABLE:
        logger.warning("MT5 not available — ETH bot running in stub mode")
        return False

    from btc_research.eth_bot.settings import MT5_LOGIN, MT5_PASSWORD, MT5_SERVER
    ...
    if info:
        logger.info("ETH MT5 connected — account %s, balance %.2f", info.login, info.balance)
    return True
...
def get_ohlcv(
    ...
    from btc_research.eth_bot.settings import MT5_SERVER_UTC_OFFSET
...
def get_live_price(symbol: str, max_age_seconds: int = 300) -> dict:
    ...
    return {
        "symbol":      symbol,
        "bid":         round(tick.bid, 4),
        "ask":         round(tick.ask, 4),
        "price":       mid,
