...
Uses dedicated ETH bot credentials:
  ETH_TELEGRAM_BOT_TOKEN  — bot token from BotFather for ETH bot
  ETH_TELEGRAM_CHAT_ID    — chat/channel ID for ETH alerts

Falls back to shared TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID if ETH-specific
vars are not set.
...
class ETHAlerter:
    ...
    def __init__(
        ...
        self._token = (
            token
            or os.environ.get("ETH_TELEGRAM_BOT_TOKEN", "")
            or os.environ.get("TELEGRAM_BOT_TOKEN", "")
        )
        self._chat_id = (
            chat_id
            or os.environ.get("ETH_TELEGRAM_CHAT_ID", "")
            or os.environ.get("TELEGRAM_CHAT_ID", "")
        )

        if not self._token:
            logger.info(
                "ETHAlerter: no token set (ETH_TELEGRAM_BOT_TOKEN / TELEGRAM_BOT_TOKEN) "
                "— alerts disabled"
            )
        else:
            src = (
                "ETH_TELEGRAM_BOT_TOKEN" if os.environ.get("ETH_TELEGRAM_BOT_TOKEN")
                else ("__init__ arg" if token else "TELEGRAM_BOT_TOKEN (shared fallback)")
            )
            logger.info(
                "ETHAlerter ready — token src: %s  |  chat_id: %s",
                src, self._chat_id or "(not set)",
            )
    ...
    def send_startup(self, balance: float) -> bool:
        ...
        msg = (
            ...
            f"Strategy: Swing Level v2 + VB (placeholder — pending backtest)\n"
