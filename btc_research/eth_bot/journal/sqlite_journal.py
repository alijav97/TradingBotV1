"""
btc_research/eth_bot/journal/sqlite_journal.py — SQLite trade journal for ETH Bot.

Fully standalone — mirrors btc_bot_2/journal/sqlite_journal.py exactly.
Changes vs btc_bot_2:
  - get_paper_balance() uses eth_bot.settings.STARTING_BALANCE
  - DB path defaults to eth_bot/data/eth_trades.db
  - Log prefixes say "ETH" not "BTC2"
  - No reference to any other bot

Schema is identical to btc_bot_2 so analysis scripts can be shared.
"""
...
class Journal:
    """SQLite trade journal for ETH Bot. Thread-safe via WAL mode."""

    def __init__(self, db_path: str | Path | None = None) -> None:
        if db_path is None:
            from btc_research.eth_bot.settings import DB_PATH
            db_path = DB_PATH
        ...
        self._init_schema()
        logger.info("ETH Journal opened: %s", self.db_path)
    ...
    def open_trade(self, trade: dict) -> str:
        ...
        self._conn.commit()
        logger.info(
            "ETH Trade opened: %s %s %s @ %.4f  SL=%.4f  lot=%.4f",
    ...
    def close_trade(
        ...
        if updated:
            logger.info(
                "ETH Trade closed: %s exit=%.4f reason=%s pnl=%s",
    ...
    def update_trade(self, trade_id: str, updates: dict) -> bool:
        ...
        Used by ETHPaperTrader for TP1 hits, BE moves, trailing SL.
        """
