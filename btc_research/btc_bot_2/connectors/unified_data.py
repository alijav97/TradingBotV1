"""
btc_research/btc_bot_2/connectors/unified_data.py — Data feed for BTC Bot 2.

Fully standalone — mirrors btc_bot_1/connectors/unified_data.py exactly.
BTC Bot 2 only needs MT5 (BTCUSD is on Pepperstone MT5).
...
Usage:
    ...
    feed = DataFeed()
    feed.connect()
    df = feed.get_ohlcv("BTCUSD", "H1", count=300)
    px = feed.get_price("BTCUSD")
"""
...
import btc_research.btc_bot_2.connectors.mt5_connector as mt5_conn
...
class DataFeed:
    """MT5-backed data feed for BTC Bot 2. Fully independent of v2."""
    ...
    def connect(
        ...
        if not self._mt5_ok:
            logger.warning("BTC2 MT5 not connected — data unavailable")
